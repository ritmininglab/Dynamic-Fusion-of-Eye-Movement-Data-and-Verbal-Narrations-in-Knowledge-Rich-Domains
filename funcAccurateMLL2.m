function marginal_scalar = funcAccurateMLL2(Npat, Nmix, Y, s0, sk, sx, priormu)


marginal_accurate = zeros(Npat,1);
for pat = 1:Npat
    nmix = Nmix(pat);
    X = cell(nmix,1);
    for i = 1:nmix
    ...
    mu = B/A;
    S = 0.5/A;
    marginal_accurate(pat) =  C + 0.5*log(S) - 0.5 * log(s0 + S)...
        - 0.5 * mu^2 / S - 0.5 * priormu^2 / s0 ...
        + 0.5 / (s0 + S) * (s0/S * mu^2 + S/s0 * priormu^2 + 2 * mu*priormu);
end
marginal_scalar = sum(marginal_accurate);
